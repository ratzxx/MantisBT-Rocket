...
class RocketPlugin extends MantisPlugin {
    var $skip = false;
    ...
    function register() {
        ...
        $this->author = 'Dmitry Litus';
        $this->contact = 'ratzxx@gmail.com';
        $this->url = 'https://vlab.su';
    }
    ...
    function bugnote_add_edit($event, $bug_id, $bugnote_id) {
        ...
        $this->notify($msg, $this->get_webhook($project), $this->get_text_attachment($this->bbcode_to_rocket($note)));
    }
    ...
    function format_text($bug, $text) {
        $t = string_display_line_links($this->bbcode_to_rocket($text));
    ...
    function bbcode_to_rocket($bbtext){
        $bbtags = array(
            '[b]' => '*','[/b]' => '* ',
            '[i]' => '_','[/i]' => '_ ',
            '[u]' => '_','[/u]' => '_ ',
